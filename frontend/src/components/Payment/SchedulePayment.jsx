...
//               {selectedBooking.schedules.map((sch, i) => {
//                 // ✅ Balance direct from API, SurplusAmount calculated
//                 const apiBalance = stripNum(sch.BalanceToRecive);
//                 const surplusAmount = Math.max(0, stripNum(sch.Amount) - apiBalance);
//                 const isComplete = apiBalance <= 0;
//                 const isScheduleOverdue = isOverdue(sch.Planned) && !isComplete;
//                 const hasSurplus = !isComplete && surplusAmount > 0;
...
//                         {/* ✅ CHANGED: Balance — direct from API BalanceToRecive */}
//                         {isComplete ? (
//                           <span className="flex items-center gap-1 text-xs text-emerald-700 font-semibold bg-emerald-100 px-2 py-0.5 rounded-full">
...
//                             Balance: {formatCurrencyShort(apiBalance)}
//                           </span>
//                         )}

//                         {/* ✅ NEW: SurplusAmount — direct from API */}
//                         {hasSurplus && (
//                           <>
//                             <span className="text-slate-400">|</span>
...
//                               <TrendingUp size={11} />
//                               Surplus: {formatCurrencyShort(surplusAmount)}
//                             </span>
//                           </>
//                         )}
...
//                       <div>
//                         <p className="text-slate-400 mb-0.5">Payment ID</p>
//                         <p className="font-mono font-semibold text-slate-700">{sch.paymentId || '—'}</p>
//                       </div>
//                       <div>
...
//                       </div>
//                       {/* ✅ CHANGED: Balance from API directly */}
//                       <div>
//                         <p className="text-slate-400 mb-0.5">Balance</p>
//                         <p className={`font-bold ${apiBalance > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
//                           {apiBalance > 0 ? formatCurrencyShort(apiBalance) : '₹0'}
//                         </p>
//                       </div>
//                       {/* ✅ NEW: SurplusAmount from API */}
//                       <div>
//                         <p className="text-slate-400 mb-0.5">Surplus Amount</p>
//                         <p className={`font-bold ${surplusAmount > 0 ? 'text-teal-600' : 'text-slate-400'}`}>
//                           {surplusAmount > 0 ? formatCurrencyShort(surplusAmount) : '—'}
//                         </p>
//                       </div>
//                       <div>
//                         <p className="text-slate-400 mb-0.5">Next Follow-up</p>
//                         <p className="font-bold text-purple-700">{fuInfo.nextDate}</p>
//                       </div>
//                     </div>
...
//                   ? selectedPayment.previousPayments.reduce((a, p) => a + getPreviousAmount(p), 0)
//                   : 0;
//                 // ✅ CHANGED: stillDue from API BalanceToRecive directly
//                 const stillDue = stripNum(selectedPayment.BalanceToRecive);
//                 const pct = schAmount > 0 ? Math.round((alreadyRec / schAmount) * 100) : 0;
//                 return (
//                   <div className="rounded-xl border border-slate-200 overflow-hidden">
//                     <div className="bg-slate-700 px-4 py-2 flex items-center gap-2">
...
const SchedulePayment = () => {
  ...
  const handleSubmitAction = async () => {
    ...
    // ✅ NEW: Total Scheduled limit check
    if (['Done', 'partial'].includes(actionForm.status)) {
      const newAmount = parseFloat(actionForm.amountReceived) || 0;
      const booking = selectedBookingForAction;
      if (booking) {
        const totalScheduled = booking.schedules.reduce((s, sch) => s + stripNum(sch.Amount), 0);
        const totalAlreadyReceived = booking.schedules.reduce((s, sch) => {
          if (sch.previousPayments?.length > 0) {
            return s + sch.previousPayments.reduce((a, p) => a + getPreviousAmount(p), 0);
          }
          return s;
        }, 0);
        const totalAfterThisPayment = totalAlreadyReceived + newAmount;
        if (totalAfterThisPayment > totalScheduled) {
          const overLimit = totalAfterThisPayment - totalScheduled;
          alert(
            `❌ Payment Limit Exceed!\n\nTotal Scheduled: ₹${totalScheduled.toLocaleString('en-IN')}\nPehle Se Received: ₹${totalAlreadyReceived.toLocaleString('en-IN')}\nYeh Amount: ₹${newAmount.toLocaleString('en-IN')}\n\nTotal ₹${overLimit.toLocaleString('en-IN')} zyada ho raha hai.\nAmount kam karein.`
          );
          return;
        }
      }
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            onClick={e => openActionModal(booking.schedules[0], e, booking)}
                            disabled={isUpdating}
                            className="p-1.5 rounded-lg bg-green-100 hover:bg-green-200 text-green-600 hover:text-green-700 transition disabled:opacity-50"
                            title="Take Action"
                          >
                  ...
                  <div key={i} className={`rounded-xl border-2 overflow-hidden shadow-sm ${isComplete ? 'border-emerald-200' : isScheduleOverdue ? 'border-red-300' : 'border-slate-200'}`}>
                    ...
                    <div className={`px-5 py-3.5 flex flex-wrap items-center justify-between gap-3 ${isComplete ? 'bg-emerald-50' : isScheduleOverdue ? 'bg-red-50' : 'bg-slate-50'} border-b border-slate-200`}>
                      ...
                      {isComplete ? (
                        ...
                        <button onClick={e => openActionModal(sch, e, selectedBooking)}
                          className="px-4 py-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded-lg transition flex items-center gap-1.5 flex-shrink-0">
                          <Edit3 size={12} /> Take Action
                        </button>
                      )}
                    ...
                    <div className="px-5 py-3 grid grid-cols-2 sm:grid-cols-5 gap-4 bg-white border-b border-slate-100 text-xs">
                      ...
                      {/* ✅ SurplusAmount — sirf tab jab isComplete false ho */}
                      <div>
                        <p className="text-slate-400 mb-0.5">Surplus Amount</p>
                        <p className={`font-bold ${hasSurplus ? 'text-teal-600' : 'text-slate-400'}`}>
                          {hasSurplus ? formatCurrencyShort(surplusAmount) : '—'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 mb-0.5">Next Follow-up</p>
