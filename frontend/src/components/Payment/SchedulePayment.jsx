...
//     // Reverse GST calc (gross is inclusive of GST)
...
//       const schAmount = stripNum(s.Amount);
//       const received = (s.previousPayments || []).reduce(
//         (acc, p) => isRefundPayment(p) ? acc : acc + getPreviousAmount(p), 0
//       );
//       return Math.max(0, schAmount - received) > 0;
//     });
...
//       (s, p) => isRefundPayment(p) ? s : s + getPreviousAmount(p), 0
//     );

//   const getScheduleTotalRefund = (sch) =>
//     (sch.previousPayments || []).reduce(
//       (s, p) => isRefundPayment(p) ? s + getPreviousAmount(p) : s, 0
//     );
...
//   const getScheduleSurplus = (sch) => {
//     const schAmount = stripNum(sch.Amount);
//     const received = getScheduleTotalReceived(sch);
//     return Math.max(0, received - schAmount);
//   };

//   const isScheduleComplete = (sch) => getScheduleBalance(sch) <= 0;

//   /* ── Booking-level helpers ── */
//   const getBookingTotalReceived = (booking) => {
//     const bookingAmt = stripNum(booking.bookingAmount);
//     const paymentsReceived = booking.schedules.reduce((sum, sch) => sum + getScheduleTotalReceived(sch), 0);
//     return bookingAmt + paymentsReceived;
//   };
...
//     booking.schedules.reduce((sum, sch) => sum + getScheduleSurplus(sch), 0);
...
//       if (plannedDate && plannedDate < today) return sum + Math.max(0, getScheduleBalance(sch));
//       return sum;
//     }, 0);
//   };
...
//       if (!groups[bookingId]) {
//         groups[bookingId] = {
//           bookingId, applicantName: payment.applicantName || '—', unitNo: payment.unitNo || '—',
//           unitCode: payment.unitCode || '—', block: payment.block || '—', unitType: payment.unitType || '—',
//           size: payment.size || '—', Project: payment.Project || '—', contact: payment.contact || '—',
//           email: payment.email || '—', CurrentAddress: payment.CurrentAddress || '—',
//           agreementValue: stripNum(payment.agreementValue), bookingAmount: stripNum(payment.bookingAmount),
...
//     });
//     Object.values(groups).forEach(g => { if (g.balanceToReceive <= 0) g.balanceToReceive = g.agreementValue - g.bookingAmount; });
//     return Object.values(groups);
//   }, [payments]);

//   const projectList = useMemo(() => { const set = new Set(groupedBookings.map(b => b.Project).filter(p => p && p !== '—')); return [...set].sort(); }, [groupedBookings]);

//   const getBookingStatus = (booking) => {
//     const pending = booking.schedules.filter(s => !isScheduleComplete(s)).length;
...
//     e?.stopPropagation();
//     setSelectedPayment(payment); setSelectedBookingForAction(booking || null);
//     setActionForm({
...
//   const closeActionModal = () => { setShowActionModal(false); setSelectedPayment(null); setSelectedBookingForAction(null); };
...
//     // Next Date aur Remark SIRF pending ke liye zaroori hai
...
//     try {
//       // For worknotdone: gross = amount + TDS
...
//       <div className="text-center"><Loader2 className="animate-spin h-12 w-12 text-blue-600 mx-auto" /><p className="mt-4 text-slate-500 font-medium">Loading payment schedules...</p></div>
//     </div>
//   );
...
//               <input type="text" placeholder="Search by name, booking ID, unit, contact, email, project..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
...
//               {overdueCount > 0 && <span className={`px-1.5 py-0.5 rounded-full text-xs font-bold ${filterOverdue ? 'bg-white text-red-600' : 'bg-red-100 text-red-700'}`}>{overdueCount}</span>}
//             </button>
//             <div className="flex items-center gap-2 flex-wrap">
//               <div className="flex items-center gap-2">
//                 <div className="relative">
//                   <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)} className="px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:border-blue-400 focus:outline-none pl-14" />
//                   <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 pointer-events-none">From</span>
//                 </div>
//                 <span className="text-slate-500 font-medium">to</span>
//                 <div className="relative">
//                   <input type="date" value={toDate} onChange={e => setToDate(e.target.value)} className="px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:border-blue-400 focus:outline-none pl-10" />
//                   <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 pointer-events-none">To</span>
//                 </div>
//               </div>
//               {(fromDate || toDate) && <button onClick={() => { setFromDate(''); setToDate(''); }} className="px-4 py-2.5 text-sm text-red-600 border border-red-200 rounded-lg hover:bg-red-50 font-medium whitespace-nowrap">Clear Dates</button>}
//             </div>
//             {(searchQuery || filterProject || filterOverdue || fromDate || toDate) && (
//               <button onClick={() => { setSearchQuery(''); setFilterProject(''); setFilterOverdue(false); setFromDate(''); setToDate(''); }}
//                 className="px-4 py-2.5 text-sm text-red-600 border border-red-200 rounded-lg hover:bg-red-50 font-medium whitespace-nowrap">Clear All</button>
//             )}
//           </div>
...
//               {filterOverdue && <span className="ml-2 text-red-600 font-semibold">(Overdue filter active)</span>}
...
//                 <span className="opacity-70">Filtered Overdue Total</span><span className="text-lg font-black">{formatCurrencyShort(filteredOverdueAmount)}</span>
//               </div>
...
//                 <span className="opacity-70">Filtered Total Received</span><span className="text-lg font-black">{formatCurrencyShort(filteredTotalReceived)}</span>
//               </div>
//             </div>
//           </div>
//         </div>
...
//                   <tr><td colSpan={18} className="py-20 text-center">
...
//                   const pendingCount = booking.schedules.filter(s => !isScheduleComplete(s)).length;
...
//                         {pendingCount > 0 ? <span className="bg-red-100 text-red-700 px-2.5 py-1 rounded-full text-xs font-semibold">{pendingCount}</span>
...
//                           <button onClick={e => openActionModal(booking.schedules[0], e, booking)} disabled={isUpdating} className="p-1.5 rounded-lg bg-green-100 hover:bg-green-200 text-green-600 hover:text-green-700 transition disabled:opacity-50" title="Take Action"><Edit3 size={14} /></button>
//                         </div>
//                       </td>
//                     </tr>
//                   );
...
//                   return <button key={page} onClick={() => setCurrentPage(page)} className={`w-8 h-8 rounded-lg text-xs font-semibold transition ${currentPage === page ? 'bg-blue-600 text-white' : 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-100'}`}>{page}</button>;
//                 })}
...
//               const pct = originalAgrValue > 0 ? Math.round((netReceived / originalAgrValue) * 100) : 0;
...
//                     <span className="ml-auto text-xs text-slate-300">{pct}% collected {hasRefund ? '(after refund)' : ''}</span>
//                   </div>
//                   <div className="overflow-x-auto">
...
//                           <p className="text-[10px] text-green-500 mt-0.5">After refund deduction</p>
//                         </div>
//                       )}
//                       {hasSurplus && (
...
//                           <p className="text-[10px] text-teal-500 mt-0.5">Overpaid across schedules</p>
//                         </div>
//                       )}
...
//                         <p className="text-[10px] text-amber-500 mt-0.5">{hasRefund ? 'Agr.Value − Net Received' : 'Agr.Value − Received'}</p>
//                       </div>
//                     </div>
//                   </div>
...
//               <h3 className="font-bold text-slate-800 flex items-center gap-2"><Calendar size={17} className="text-blue-600" />Payment Schedules ({selectedBooking.schedules.length})</h3>
...
//                 const balance = getScheduleBalance(sch);
//                 const surplusAmount = getScheduleSurplus(sch);
//                 const isComplete = isScheduleComplete(sch);
//                 const fmsSurplus = stripNum(sch.SurPlusAmount);
//                 const displaySurplus = surplusAmount > 0 ? surplusAmount : fmsSurplus;
//                 const isScheduleOverdue = isOverdue(sch.Planned) && !isComplete;
//                 const hasSurplus = displaySurplus > 0;
...
//                               <TrendingUp size={11} />Surplus: {formatCurrencyShort(displaySurplus)}
//                             </span>
//                           </>
//                         )}
//                         {totalRefundSch > 0 && (
...
//                       </div>
//                       {!isComplete && (
//                         <button onClick={e => openActionModal(sch, e, selectedBooking)} className="px-4 py-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded-lg transition flex items-center gap-1.5 flex-shrink-0"><Edit3 size={12} /> Take Action</button>
//                       )}
//                     </div>

//                     <div className="px-5 py-3 grid grid-cols-2 sm:grid-cols-6 gap-4 bg-white border-b border-slate-100 text-xs">
...
//                       <div>
//                         <p className="text-slate-400 mb-0.5">Total Received</p>
//                         <p className="font-bold text-emerald-700">{totalReceivedSch > 0 ? formatCurrencyShort(totalReceivedSch) : '—'}</p>
//                       </div>
//                       <div>
//                         <p className="text-slate-400 mb-0.5">Balance</p>
//                         <p className={`font-bold ${balance > 0 ? 'text-red-600' : 'text-emerald-600'}`}>{balance > 0 ? formatCurrencyShort(balance) : '₹0'}</p>
//                       </div>
//                       <div>
//                         <p className="text-slate-400 mb-0.5">Surplus</p>
//                         <p className={`font-bold ${displaySurplus > 0 ? 'text-teal-700' : 'text-slate-400'}`}>
//                           {displaySurplus > 0 ? <span className="flex items-center gap-0.5"><TrendingUp size={10} />{formatCurrencyShort(displaySurplus)}</span> : '—'}
//                         </p>
//                       </div>
//                       <div>
//                         <p className="text-slate-400 mb-0.5">Refunded</p>
//                         <p className={`font-bold ${totalRefundSch > 0 ? 'text-orange-600' : 'text-slate-400'}`}>{totalRefundSch > 0 ? `−${formatCurrencyShort(totalRefundSch)}` : '—'}</p>
//                       </div>
...
//                                     {formatCurrencyShort(sch.previousPayments.reduce((s, p) => isRefundPayment(p) ? s : s + (parseFloat(p.grossAmount) || 0), 0))}
//                                   </td>
//                                   <td className="px-4 py-2.5 text-right font-bold text-orange-600 whitespace-nowrap">
//                                     {formatCurrencyShort(sch.previousPayments.reduce((s, p) => isRefundPayment(p) ? s : s + (parseFloat(p.cgst) || 0), 0))}
//                                   </td>
//                                   <td className="px-4 py-2.5 text-right font-bold text-purple-600 whitespace-nowrap">
//                                     {formatCurrencyShort(sch.previousPayments.reduce((s, p) => isRefundPayment(p) ? s : s + (parseFloat(p.sgst) || 0), 0))}
//                                   </td>
//                                   <td className="px-4 py-2.5 text-right font-black text-emerald-700 whitespace-nowrap">{formatCurrencyShort(totalReceivedSch)}</td>
//                                   <td colSpan={3}></td>
//                                 </tr>
//                                 {totalRefundSch > 0 && (
//                                   <tr className="bg-orange-50 border-t border-orange-200">
//                                     <td colSpan={2} className="px-4 py-2 text-xs font-bold text-orange-700">
//                                       <span className="flex items-center gap-1"><ArrowDownLeft size={11} />Total Refunded (Company→Client)</span>
//                                     </td>
//                                     <td className="px-4 py-2 text-right font-bold text-orange-600 text-xs whitespace-nowrap">
//                                       −{formatCurrencyShort(sch.previousPayments.reduce((s, p) => isRefundPayment(p) ? s + (parseFloat(p.grossAmount) || 0) : s, 0))}
//                                     </td>
//                                     <td colSpan={2} className="px-4 py-2 text-right text-xs text-orange-400">(GST na-applicable)</td>
//                                     <td className="px-4 py-2 text-right font-black text-orange-700 text-xs whitespace-nowrap">−{formatCurrencyShort(totalRefundSch)}</td>
//                                     <td colSpan={3}></td>
//                                   </tr>
//                                 )}
//                               </>
...
//                 const surplusAmt = getScheduleSurplus(selectedPayment);
//                 const fmsSurplus = stripNum(selectedPayment.SurPlusAmount);
//                 const displaySurplus = surplusAmt > 0 ? surplusAmt : fmsSurplus;
...
//                       </div>
//                       <div className={`px-4 py-3 ${displaySurplus > 0 ? 'bg-teal-50' : ''}`}>
//                         <p className="text-xs text-teal-600 uppercase tracking-wide mb-1 flex items-center gap-1"><TrendingUp size={10} /> Surplus</p>
//                         <p className={`font-black text-base ${displaySurplus > 0 ? 'text-teal-700' : 'text-slate-400'}`}>
//                           {displaySurplus > 0 ? formatCurrencyShort(displaySurplus) : '₹0'}
//                         </p>
//                         <p className="text-xs text-teal-500 mt-0.5">{displaySurplus > 0 ? 'Overpaid' : 'No surplus'}</p>
//                       </div>
//                     </div>
...
//                     {/* {!isCRM && <option value="Done">✓ Done</option>} */}
...
//                 {/* ── Work Not Done Info Box ── */}
//                 {isWorkNotDoneStatus(actionForm.status) && (
//                   <div className="md:col-span-2 bg-yellow-50 border-2 border-yellow-200 rounded-xl p-4">
//                     <div className="flex items-start gap-3">
//                       <FileText size={20} className="text-yellow-600 flex-shrink-0 mt-0.5" />
//                       <div>
//                         <p className="font-bold text-yellow-800 text-sm">Work Not Done — Payment with TDS</p>
//                         <ul className="mt-2 space-y-1 text-xs text-yellow-700">
//                           <li>• Amount aur TDS alag-alag enter karein</li>
//                           <li>• <strong>Gross = Amount + TDS</strong></li>
//                           <li>• GST agar applicable hai to Gross pe calculate hogi</li>
//                           <li>• TDS value sheet ke X column me store hogi</li>
//                         </ul>
//                       </div>
//                     </div>
//                   </div>
...
//                 {/* ── Refund Info Box ── */}
//                 {isRefundStatus(actionForm.status) && (
//                   <div className="md:col-span-2 bg-orange-50 border-2 border-orange-200 rounded-xl p-4">
//                     <div className="flex items-start gap-3">
//                       <ArrowDownLeft size={20} className="text-orange-600 flex-shrink-0 mt-0.5" />
//                       <div>
//                         <p className="font-bold text-orange-800 text-sm">Refund Payment — Company → Client</p>
//                         <ul className="mt-2 space-y-1 text-xs text-orange-700">
//                           <li>• Refund amount <strong>Total Received</strong> se minus hoga</li>
//                           <li>• <strong>Net Received = Total Received − Refund</strong></li>
//                           <li>• Agreement Value same rahegi (koi change nahi)</li>
//                           <li>• Balance Due = Agreement Value − Net Received</li>
//                           <li>• Payment sheet me <strong>AC column</strong> me "refund" status jayega</li>
//                         </ul>
//                       </div>
//                     </div>
//                   </div>
...
//                 {/* ── Payment Fields (Done, Partial, Refund, WorkNotDone) ── */}
...
//                         className={`w-full px-4 py-2.5 border-2 rounded-lg focus:outline-none text-sm ${isRefundStatus(actionForm.status) ? 'border-orange-300 focus:border-orange-400' : isWorkNotDoneStatus(actionForm.status) ? 'border-yellow-300 focus:border-yellow-400' : 'border-slate-200 focus:border-blue-400'}`} />
//                     </FormField>
...
//                           className={`w-full pl-8 pr-4 py-2.5 border-2 rounded-lg focus:outline-none text-sm ${isRefundStatus(actionForm.status) ? 'border-orange-300 focus:border-orange-400 bg-orange-50' : isWorkNotDoneStatus(actionForm.status) ? 'border-yellow-300 focus:border-yellow-400 bg-yellow-50' : 'border-slate-200 focus:border-blue-400'}`} />
//                       </div>
//                     </FormField>

//                     {/* ── TDS Field — ONLY for Work Not Done ── */}
//                     {isWorkNotDoneStatus(actionForm.status) && (
//                       <FormField label="TDS Amount" required>
//                         <div className="relative">
//                           <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm">₹</span>
//                           <input
//                             type="number"
//                             name="tdsAmount"
//                             value={actionForm.tdsAmount}
//                             onChange={handleFormChange}
//                             placeholder="0"
//                             min="0"
//                             disabled={isUpdating}
//                             className="w-full pl-8 pr-4 py-2.5 border-2 border-yellow-300 focus:border-yellow-500 bg-yellow-50 rounded-lg focus:outline-none text-sm"
//                           />
//                         </div>
//                         <p className="text-xs text-yellow-600 mt-1">TDS amount jo gross me add hogi (Amount + TDS = Gross)</p>
//                       </FormField>
//                     )}
...
//                     {/* ── GST Breakdown ── */}
//                     {(parseFloat(actionForm.amountReceived) > 0 || (isWorkNotDoneStatus(actionForm.status) && parseFloat(actionForm.tdsAmount) > 0)) && (
//                       <div className="md:col-span-2">
//                         <div className={`rounded-xl border-2 overflow-hidden ${isRefundStatus(actionForm.status) ? 'border-orange-200' : isWorkNotDoneStatus(actionForm.status) ? 'border-yellow-200' : 'border-blue-200'}`}>
//                           <div className={`px-4 py-2 flex items-center gap-2 ${isRefundStatus(actionForm.status) ? 'bg-orange-600' : isWorkNotDoneStatus(actionForm.status) ? 'bg-yellow-600' : 'bg-blue-600'}`}>
//                             <Percent size={14} className="text-white/70" />
//                             <span className="text-xs font-bold text-white uppercase tracking-wide">
//                               {isRefundStatus(actionForm.status) ? 'Refund GST Breakdown' : isWorkNotDoneStatus(actionForm.status) ? 'Amount + TDS + GST Breakdown' : 'GST Calculation Breakdown'}
//                             </span>
//                             <span className="ml-auto text-xs text-white/70">GST: {actionForm.gstPercent}%</span>
//                           </div>
//                           <div className={`grid divide-x divide-slate-100 bg-white ${isWorkNotDoneStatus(actionForm.status) ? 'grid-cols-2 sm:grid-cols-5' : 'grid-cols-2 sm:grid-cols-4'}`}>
//                             <div className="px-4 py-3 text-center">
//                               <p className="text-xs text-slate-500 uppercase tracking-wide mb-1">Amount</p>
//                               <p className={`font-bold text-sm ${isRefundStatus(actionForm.status) ? 'text-orange-700' : 'text-slate-800'}`}>
//                                 {isRefundStatus(actionForm.status) ? '−' : ''}{formatCurrency(parseFloat(actionForm.amountReceived) || 0)}
//                               </p>
//                             </div>
//                             {/* TDS column — only for worknotdone */}
//                             {isWorkNotDoneStatus(actionForm.status) && (
//                               <div className="px-4 py-3 text-center bg-yellow-50/60">
//                                 <p className="text-xs text-yellow-600 uppercase tracking-wide mb-1">+ TDS</p>
//                                 <p className="font-bold text-yellow-700 text-sm">{formatCurrency(gstCalculation.tds)}</p>
//                               </div>
//                             )}
...
//                               <p className={`font-black text-base ${isRefundStatus(actionForm.status) ? 'text-orange-700' : 'text-emerald-700'}`}>
//                                 {isRefundStatus(actionForm.status) ? '−' : ''}{formatCurrency(gstCalculation.netAmount)}
//                               </p>
//                             </div>
//                           </div>
...
//                             <span>
//                               {isWorkNotDoneStatus(actionForm.status) && (
//                                 <>Gross (Amt+TDS): <strong className="text-slate-700">{formatCurrency(gstCalculation.grossAmount)}</strong> | </>
//                               )}
//                               Total GST: <strong className="text-slate-700">{formatCurrency(gstCalculation.totalGst)}</strong>
//                             </span>
...
//                       <input type="text" name="paymentDetails" value={actionForm.paymentDetails} onChange={handleFormChange} placeholder="e.g. UTR123456, CHQ00789" disabled={isUpdating}
//                         className="w-full px-4 py-2.5 border-2 border-slate-200 rounded-lg focus:border-blue-400 focus:outline-none text-sm" />
//                     </FormField>
//                   </>
//                 )}

//                 {/* ── Next Date & Remark — ONLY for Pending (and CRM) ── */}
...
//                         placeholder={isCRM ? "Follow-up notes, customer response..." : "Reason for pending..."}
//                         disabled={isUpdating} className="w-full px-4 py-2.5 border-2 border-slate-200 rounded-lg focus:border-blue-400 focus:outline-none text-sm resize-none" />
//                     </FormField>
//                   </>
//                 )}
...
//                 {/* ── Refund Reason ── */}
//                 {isRefundStatus(actionForm.status) && (
//                   <FormField label="Refund Reason" required className="md:col-span-2">
//                     <textarea name="remark" value={actionForm.remark} onChange={handleFormChange} rows={3}
//                       placeholder="Refund ka reason — e.g. booking cancellation, customer request, overbooking..."
//                       disabled={isUpdating} className="w-full px-4 py-2.5 border-2 border-orange-200 bg-orange-50 rounded-lg focus:border-orange-400 focus:outline-none text-sm resize-none" />
//                   </FormField>
//                 )}

//                 {/* ── Work Not Done Remark (Optional) ── */}
//                 {isWorkNotDoneStatus(actionForm.status) && (
//                   <FormField label="Remark / Notes (Optional)" className="md:col-span-2">
//                     <textarea name="remark" value={actionForm.remark} onChange={handleFormChange} rows={3}
//                       placeholder="Work not done ka reason ya additional notes..."
//                       disabled={isUpdating} className="w-full px-4 py-2.5 border-2 border-yellow-200 bg-yellow-50 rounded-lg focus:border-yellow-400 focus:outline-none text-sm resize-none" />
//                   </FormField>
//                 )}

//                 {/* ── Done / Partial Remark (Optional) ── */}
//                 {(actionForm.status === 'Done' || actionForm.status === 'partial') && (
//                   <FormField label="Remark / Notes (Optional)" className="md:col-span-2">
//                     <textarea name="remark" value={actionForm.remark} onChange={handleFormChange} rows={2}
//                       placeholder={actionForm.status === 'partial' ? "Reason for partial payment..." : "Any notes..."}
//                       disabled={isUpdating} className="w-full px-4 py-2.5 border-2 border-slate-200 rounded-lg focus:border-blue-400 focus:outline-none text-sm resize-none" />
//                   </FormField>
//                 )}
//               </div>
...
//                   isRefundStatus(actionForm.status) ? 'bg-orange-600 hover:bg-orange-700' :
//                   isWorkNotDoneStatus(actionForm.status) ? 'bg-yellow-600 hover:bg-yellow-700' :
//                   'bg-green-600 hover:bg-green-700'
//                 }`}>
//                 {isUpdating ? <><Loader2 size={15} className="animate-spin" /> Submitting...</>
//                   : isRefundStatus(actionForm.status) ? <><ArrowDownLeft size={15} /> Process Refund</>
//                   : isWorkNotDoneStatus(actionForm.status) ? <><FileText size={15} /> Submit WND</>
//                   : 'Submit Action'}
//               </button>
//             </div>
//           </div>
